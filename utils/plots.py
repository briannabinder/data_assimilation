...
def plot_particles(models):
    ...
    def plot(time, ensemble_size, sigma_min_x, sigma_y, kde_type):
        ...
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 6), dpi=300, sharex=True, sharey=True)
        # fig.suptitle(f"Particle Behavior at t={time}")
        plt.rcParams['axes.labelsize'] = 50  # Axis labels size

        ax1.scatter(updated_states[time_idx - 1, :, X], updated_states[time_idx - 1, :, Y], edgecolors=input_clr, marker='o', facecolors='none', s=particle_size, alpha=0.7, label=r'$\mathbf{x}_{t-1} \sim \pi(\mathbf{x}_{t-1} | \hat{\mathbf{y}}_{1:t-1})$')
        ax1.scatter(predicted_states[time_idx, :, X], predicted_states[time_idx, :, Y], edgecolors=pred_clr, marker='o', facecolors='none', s=particle_size, alpha=0.7, label=r"$\mathbf{x}_t \sim \pi(\mathbf{x}_{t} | \hat{\mathbf{y}}_{1:t-1})$")
        ax1.scatter(updated_states[time_idx, :, X], updated_states[time_idx, :, Y], edgecolors=upd_clr, marker='o', facecolors='none', s=particle_size, alpha=0.7, label=r"$\mathbf{x}_t \sim \pi(\mathbf{x}_{t} | \hat{\mathbf{y}}_{1:t})$")
        # ax1.scatter(updated_means[time_idx][X], updated_means[time_idx][Y], c='red', s=ref_size, label=f"Updated State Mean")
        ax1.scatter(observation[time_idx - 1][X], observation[time_idx - 1][Y], c='black', marker=obs_marker, s=obs_size, label=r"$\hat{\mathbf{y}}_t$")
        ax1.scatter(reference[time_idx][X], reference[time_idx][Y], c='blue', marker=ref_marker, s=ref_size, label=f"Reference")
        
        ax2.scatter(updated_states[time_idx - 1, :, X], updated_states[time_idx - 1, :, Z], edgecolors=input_clr, marker='o', facecolors='none', s=particle_size, alpha=0.7, label=r"\x \sim \pi(x_{t-1} | \hat{y}_{1:t-1})")
        ax2.scatter(predicted_states[time_idx, :, X], predicted_states[time_idx, :, Z], edgecolors=pred_clr, marker='o', facecolors='none', s=particle_size, alpha=0.7, label=f"Predicted States")
        ax2.scatter(updated_states[time_idx, :, X], updated_states[time_idx, :, Z], edgecolors=upd_clr, marker='o', facecolors='none', s=particle_size, alpha=0.7, label=f"Updated States")
        # ax2.scatter(updated_means[time_idx][X], updated_means[time_idx][Z], c='red', s=ref_size, label=f"Updated State Mean")
        ax2.scatter(observation[time_idx - 1][X], observation[time_idx - 1][Z], c='black', marker=obs_marker, s=obs_size, label=f"Observation")
        ...
        ax1.set_xlabel("x", fontsize=axes_fontsize)
        ax1.set_ylabel("y", fontsize=axes_fontsize)
        ax1.grid(alpha=0.2)
        ax1.tick_params(axis='both', labelsize=tick_fontsize)
        ax1.xaxis.set_major_locator(MaxNLocator(integer=True))
        ax1.legend()
        
        ax2.set_xlabel("x", fontsize=axes_fontsize)
        ax2.set_ylabel("z", fontsize=axes_fontsize)
        ax2.grid(alpha=0.2)
    ...
    def save_plot(_):
        ...
        filename = f"L63_{kde_type_widget.value}_M{ensemble_size_widget.value}_SX{sigma_min_x_widget.value}_SY{sigma_y_widget.value}.pdf"
    ...
    # Interactive widgets
    time_widget = widgets.FloatSlider(min=t_min_start, max=t_min_end, step=0.1, value=429, description="Start Time")
...
def plot_timeplots(models):
    ...
    filter_keys = ["KDE VE", "KDE VP"]
    if models.get("EnKF") is not None: 
        filter_keys.append("EnKF")
        enkf_available = True


    fig, axes = None, None  # Global figure and axes for saving

    def plot(t_min, ensemble_size, sigma_min_x, sigma_y, kde_type):
        """
        Inner function to generate and display a time series plot for a selected model.
        ...
        if kde_type != "EnKF":
            model = models[kde_type][(ensemble_size, sigma_min_x, sigma_y)]
        # elif kde_type == "EnKF":
        #     model = models[kde_type][ensemble_size]

        # Create a figure with 3 subplots, one for each variable
        fig, ax = plt.subplots(1, 1, figsize=(10, 5), dpi=300, sharex=True, constrained_layout=True)
        ...
        ax.plot(model.reference_times, ref_data, linewidth=1, label='Reference', color='blue')
        ax.plot(model.times, update_data, c='Red', marker='o', linestyle='--', markersize=4, zorder=2, label=f'CD Posterior Mean')
        ax.errorbar(model.times, update_data, yerr=std_upd, fmt='none', c="Grey", zorder=0, capsize=3)

            
        if enkf_available:
            enkf_model = models["EnKF"][ensemble_size]
            enkf_update_data = enkf_model.mean_updates[:, idx]
            ax.plot(enkf_model.times, enkf_update_data, c='Green', marker='o', linestyle='--', markersize=4, zorder=2, label=f'EnKF Posterior Mean')
            ax.errorbar(enkf_model.times, enkf_update_data, yerr=std_upd, fmt='none', c="Grey", zorder=0, capsize=3)

        ax.legend(fontsize=legend_fontsize, loc='upper center', bbox_to_anchor=(0.66, 0.3),)

        ax.set_ylabel(f'x(t)', fontsize=axes_fontsize)
        ax.set_ylim(-20, 20)
        ax.grid(alpha=0.4)

        ax.set_xlabel('Time (t)', fontsize=axes_fontsize)
        plt.xlim(t_min, t_min + 5)
        ax.tick_params(axis='both', labelsize=legend_fontsize)

        # Create the zoomed-in inset axes
        # zoom_x_min, zoom_x_max = 403, 403.5  # Define the x-range for zoom
        # zoom_y_min, zoom_y_max = -0.5, 5  # Define the y-range for zoom

        # axins = zoomed_inset_axes(ax, zoom=3, loc='upper left', bbox_to_anchor=(0.02, 0.98), bbox_transform=ax.transAxes)

        # axins.plot(model.reference_times, ref_data, linewidth=1, label='Reference', color='blue')
        # axins.plot(model.times, update_data, c='Red', marker='o', linestyle='--', markersize=4, zorder=2, label=f'{kde_type} Posterior Mean')
        # axins.errorbar(model.times, update_data, yerr=std_upd, fmt='none', c="Grey", zorder=0, capsize=3)
        # if enkf_available:
        #     axins.plot(enkf_model.times, enkf_update_data, c='Green', marker='o', linestyle='--', markersize=4, zorder=2, label=f'{kde_type} Posterior Mean')
        #     axins.errorbar(enkf_model.times, enkf_update_data, yerr=std_upd, fmt='none', c="Grey", zorder=0, capsize=3)
    ...
    sigma_min_x_widget = widgets.Dropdown(options=sigma_min_xs, value=default_sigma_x, description="\u03C3_x")
    sigma_y_widget = widgets.Dropdown(options=sigma_ys, value=default_sigma_y, description="\u03C3_y")
    kde_type_widget = widgets.Dropdown(options=filter_keys, value="KDE VE", description="KDE Type")
    # show_enkf_widget = widgets.Checkbox(value=False, description="Show EnKF Positions")
    save_button = widgets.Button(description="Save Plot", button_style='success')
    save_button.on_click(save_plot)
...
def plot_rmses(models):
    ...
    sigma_xs_num = [x for x in sigma_min_xs if isinstance(x, (int, float))]
    sigma_xs_str = [x for x in sigma_min_xs if isinstance(x, str)]
    sigma_min_xs = np.array(sorted(sigma_xs_num) + sigma_xs_str)

    # Default values for sigma_min_x and sigma_y (used for initial plot display)
    default_sigma_x = sigma_min_xs[0]
    default_sigma_y = sigma_ys[0]

    def plot(sigma_min_x, sigma_y, zoom):
        """
        ...
        for ensemble_size in ensemble_sizes:
            ...
            if ensemble_size == 600:
                print(f"KDE VE RMSE = {np.mean(kde_ve_models[(ensemble_size, sigma_min_x, sigma_y)].rmses)}")
                print(f"KDE VP RMSE = {np.mean(kde_vp_models[(ensemble_size, sigma_min_x, sigma_y)].rmses)}")
                # print(f"EnKF = {np.mean(models["EnKF"][ensemble_size].rmses)}")
        ...
        ax.set_xlabel("Ensemble Size", fontsize=10)
        ax.set_ylabel("Average RMSE", fontsize=10)
        if zoom == True:
...
def get_kde_keys(models):
    ...
    sigma_min_xs = list(set(key[1] for key in models.keys()))
